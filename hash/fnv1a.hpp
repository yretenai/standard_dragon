...
namespace dragon::hash {
    ...
    uint32_t fnv1a32(const uint8_t* buf, size_t size, uint32_t basis = FNV_BASIS_32, uint32_t prime = FNV_PRIME_32) {
    ...
    uint64_t fnv1a64(const uint8_t* buf, size_t size, uint64_t basis = FNV_BASIS_64, uint64_t prime = FNV_PRIME_64) {
        for(size_t i = 0; i < size; ++i) {
            basis ^= buf[i];
            basis *= prime;
        }
